...
We have researched the competition of two layer networks. By changing network structures, switching updating rules and finding key nodes, the features of competition on two layers are found out. We hope that deficiency of this research would be researched forward. 
\section{Summary}
To understand the dynamics of competition on interconnected network, competing interconnected networks were introduced to have different dynamics on each layer. And some indexes were provided to measure how the network state is changed and evaluate the consensus on two layer. Based on this modeling, various simulations were implemented according to 3 main topics such as changing network structures, switching updating rules and finding key nodes.
...
In chapter 3, we have investigated competition on two layer with various structural network. With changing network structure, it was measured that how the interconnected network change its state and make consensus. As the method to revise the network structure, 3 ways were provided. First, as the result of changing the internal degrees, a layer with more internal edges has more tendency to keep its own states
