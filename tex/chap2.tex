...
\end{figure}
Figure.~\ref{chap2_CI_values} shows the characteristics of \textit{CI}. Same orientation in two layers makes \texit{CI} 0. Opposite orientation between two layers makes \texit{CI} 1. And Mixed orientation in two layers makes \texit{CI} close to 0.5. 
...
\end{figure}
As Figure.~\ref{chap2_timeflow_CI(BA3_BA3)} shown, \texit{CI} values are convergent to $+1$, $0$, or other values as step(time) goes by. $0$ means positive or negative consensus. $+1$ means opposite state between two layers. The other values means mixed state. By using \texit{CI}, coexistence states can be divided into two categories, opposite state and mixed state.   
